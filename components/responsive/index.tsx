...
export const ResponsiveGrid = forwardRef<HTMLDivElement, ResponsiveGridProps>(
  ({ children, cols = { mobile: 1, tablet: 2, desktop: 3 }, gap = 'md', className, ...props }, ref) => {
    const gapClasses = {
      sm: 'gap-4',
      md: 'gap-6',
      lg: 'gap-8',
...
export const ResponsiveText = forwardRef<HTMLElement, ResponsiveTextProps>(
  ({ children, as: Component = 'p', size = 'base', weight = 'normal', className, ...props }, ref) => {
    ...
    const sizeClasses = {
      ...
      '3xl': isMobile ? 'text-3xl' : 'text-3xl md:text-4xl lg:text-5xl xl:text-6xl'
    }
...
export const ResponsiveSection = forwardRef<HTMLElement, ResponsiveSectionProps>(
  ({ children, spacing = 'md', background = 'default', className, ...props }, ref) => {
    ...
    return (
      <div
        ...
        className={cn(
          ...
          paddingClasses[padding],
          hover && 'hover:border-primary/20 hover:bg-card/80 transition-all duration-300',
          className
        )}
        {...props}
      >
