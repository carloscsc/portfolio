    ...
    <div 
      ...
      <div className="mt-1 flex justify-end">
        <span className={`text-xs text-muted-foreground ${
          ...
          percentage >= 60 ? 'text-yellow-500' : 'text-gray-500'
        }`}>
          {percentage >= 90 ? 'Avançado' :
           percentage >= 75 ? 'Intermediário+' :
           percentage >= 60 ? 'Intermediário' : 'Básico'}
        </span>
      </div>
    </div>
  )
    ...
    <section 
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-gray-400 max-w-2xl mx-auto text-responsive-base leading-relaxed px-4">
          ...
          Aqui está uma visão geral das minhas competências técnicas e níveis de proficiência.
        </p>
      </div>
      ...
      {isMobile && isVisible && (
        <div className="mt-8 text-center">
          <p className="text-xs text-muted-foreground">
            {skills.filter(skill => skill.percentage >= 90).length} habilidades avançadas • {' '}
            {skills.filter(skill => skill.percentage >= 75 && skill.percentage < 90).length} intermediárias+
          </p>
        </div>
      )}
    </section>
